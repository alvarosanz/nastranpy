...
card_interfaces = {
    ...
    'CORD2S': class_factory('CORD2S', Item.coord, [F(), F(), F('RID', Item.coord), F('A', seq_type=Seq.vector, length=3), F('B', seq_type=Seq.vector, length=3),
                                                   F('C', seq_type=Seq.vector, length=3)]),
    'CORD1R': class_factory('CORD1R', Item.coord, [F(), F(), F('G1A', Item.grid), F('G2A', Item.grid), F('G3A', Item.grid), F(other_card=True)]),
    'CORD1C': class_factory('CORD1C', Item.coord, [F(), F(), F('G1A', Item.grid), F('G2A', Item.grid), F('G3A', Item.grid), F(other_card=True)]),
    'CORD1S': class_factory('CORD1S', Item.coord, [F(), F(), F('G1A', Item.grid), F('G2A', Item.grid), F('G3A', Item.grid), F(other_card=True)]),
    # Materials
    ...
    # Elements
    'CROD': class_factory('CROD', Item.elem, [F(), F(), F(Item.prop.name, Item.prop), F('grids', Item.grid, seq_type=Seq.list, length=2, update_grid=True)], card_tag=Tag.e1D),
    'CONROD': class_factory('CONROD', Item.elem, [F(), F(), F('grids', Item.grid, seq_type=Seq.list, length=2, update_grid=True), F(Item.mat.name, Item.mat), F('A'), F('J'), F('C'), F('NSM')], card_tag=Tag.e1D),
    'CBAR': class_factory('CBAR', Item.elem, [F(), F(), F(Item.prop.name, Item.prop), F('grids', Item.grid, seq_type=Seq.list, length=2, update_grid=True), F('v', Item.grid, seq_type=Seq.vector, length=3, alternate_name='G0'), F('OFFT'),
                                              F('PA'), F('PB'), F('offsetA', seq_type=Seq.vector, length=3), F('offsetB', seq_type=Seq.vector, length=3)], card_tag=Tag.e1D),
    'CBEAM': class_factory('CBEAM', Item.elem, [F(), F(), F(Item.prop.name, Item.prop), F('grids', Item.grid, seq_type=Seq.list, length=2, update_grid=True), F('v', Item.grid, seq_type=Seq.vector, length=3, alternate_name='G0'), F('OFFT'),
                                                F('PA'), F('PB'), F('offsetA', seq_type=Seq.vector, length=3), F('offsetB', seq_type=Seq.vector, length=3),
                                                F('SA', Item.grid), F('SB', Item.grid)], card_tag=Tag.e1D),
    'CQUAD4': class_factory('CQUAD4', Item.elem, [F(), F(), F(Item.prop.name, Item.prop), F('grids', Item.grid, seq_type=Seq.list, length=4, update_grid=True), F('THETA', Item.coord, alternate_name='MCID'), F('ZOFFS'),
                                                  F(), F('TFLAG'), F('T1'), F('T2'), F('T3'), F('T4')], card_tag=Tag.e2D),
    'CTRIA3': class_factory('CTRIA3', Item.elem, [F(), F(), F(Item.prop.name, Item.prop), F('grids', Item.grid, seq_type=Seq.list, length=3, update_grid=True), F('THETA', Item.coord, alternate_name='MCID'), F('ZOFFS'),
                                                  F(), F('TFLAG'), F('T1'), F('T2'), F('T3')], card_tag=Tag.e2D),
    'CTETRA': class_factory('CTETRA', Item.elem, [F(), F(), F(Item.prop.name, Item.prop), F('grids', Item.grid, seq_type=Seq.list, length=10, update_grid=True)], card_tag=Tag.e3D),
    'CPENTA': class_factory('CPENTA', Item.elem, [F(), F(), F(Item.prop.name, Item.prop), F('grids', Item.grid, seq_type=Seq.list, length=15, update_grid=True)], card_tag=Tag.e3D),
    'CHEXA': class_factory('CHEXA', Item.elem, [F(), F(), F(Item.prop.name, Item.prop), F('grids', Item.grid, seq_type=Seq.list, length=20, update_grid=True)], card_tag=Tag.e3D),
    'RBE2': class_factory('RBE2', Item.elem, [F(), F(), F('master_grid', Item.grid, update_grid=True), F('CM'), F('slave_grids', Item.grid, seq_type=Seq.set, update_grid=True), F('ALPHA')], card_tag=Tag.eRigid),
