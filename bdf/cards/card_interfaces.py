...
card_interfaces = {
    ...
    # Elements
    'CROD': class_factory('CROD', Item.elem, [F(), F(), F(Item.prop.name, Item.prop), F('grids', Item.grid, Seq.list, 2, update_grid=True, observed=True)], card_tag=Tag.e1D),
    'CONROD': class_factory('CONROD', Item.elem, [F(), F(), F('grids', Item.grid, Seq.list, 2, update_grid=True, observed=True), F(Item.mat.name, Item.mat), F('A'), F('J'), F('C'), F('NSM')], card_tag=Tag.e1D),
    'CBAR': class_factory('CBAR', Item.elem, [F(), F(), F(Item.prop.name, Item.prop), F('grids', Item.grid, Seq.list, 2, update_grid=True, observed=True), F('X1', Item.grid, alternate_name='G0', observed=True), F('X2'), F('X3'), F('OFFT'),
                                              F('PA'), F('PB'), F('W1A'), F('W2A'), F('W3A'), F('W1B'), F('W2B'), F('W3B')], card_tag=Tag.e1D),
    'CBEAM': class_factory('CBEAM', Item.elem, [F(), F(), F(Item.prop.name, Item.prop), F('grids', Item.grid, Seq.list, 2, update_grid=True, observed=True), F('X1', Item.grid, alternate_name='G0', observed=True), F('X2'), F('X3'), F('OFFT'),
                                                F('PA'), F('PB'), F('W1A'), F('W2A'), F('W3A'), F('W1B'), F('W2B'), F('W3B'),
                                                F('SA', Item.grid), F('SB', Item.grid)], card_tag=Tag.e1D),
    'CQUAD4': class_factory('CQUAD4', Item.elem, [F(), F(), F(Item.prop.name, Item.prop), F('grids', Item.grid, Seq.list, 4, update_grid=True, observed=True), F('THETA', Item.coord, alternate_name='MCID'), F('ZOFFS'),
                                                  F(), F('TFLAG'), F('T1'), F('T2'), F('T3'), F('T4')], card_tag=Tag.e2D),
    'CTRIA3': class_factory('CTRIA3', Item.elem, [F(), F(), F(Item.prop.name, Item.prop), F('grids', Item.grid, Seq.list, 3, update_grid=True, observed=True), F('THETA', Item.coord, alternate_name='MCID'), F('ZOFFS'),
                                                  F(), F('TFLAG'), F('T1'), F('T2'), F('T3')], card_tag=Tag.e2D),
    'CTETRA': class_factory('CTETRA', Item.elem, [F(), F(), F(Item.prop.name, Item.prop), F('grids', Item.grid, Seq.list, 10, update_grid=True, observed=True)], card_tag=Tag.e3D),
    'CPENTA': class_factory('CPENTA', Item.elem, [F(), F(), F(Item.prop.name, Item.prop), F('grids', Item.grid, Seq.list, 15, update_grid=True, observed=True)], card_tag=Tag.e3D),
    'CHEXA': class_factory('CHEXA', Item.elem, [F(), F(), F(Item.prop.name, Item.prop), F('grids', Item.grid, Seq.list, 20, update_grid=True, observed=True)], card_tag=Tag.e3D),
    'RBE2': class_factory('RBE2', Item.elem, [F(), F(), F('master_grid', Item.grid, update_grid=True, observed=True), F('CM'), F('slave_grids', Item.grid, Seq.set, update_grid=True, observed=True), F('ALPHA')], card_tag=Tag.eRigid),
    'RBE3': class_factory('RBE3', Item.elem, [F(), F(), F(), F('master_grid', Item.grid, update_grid=True, observed=True), F('REFC'),
                                              F('slaves', seq_type=Seq.list, subscheme=[F('WT'), F('C'), F('grids', Item.grid, Seq.set, update_grid=True, observed=True)]),
                                              F('UM', seq_type=Seq.list, subscheme=[F('GM', Item.grid), F('CM')], optional=True),
                                              F('ALPHA', optional=True)], card_tag=Tag.eRigid),
    'CBUSH': class_factory('CBUSH', Item.elem, [F(), F(), F(Item.prop.name, Item.prop), F('grids', Item.grid, Seq.list, 2, update_grid=True, observed=True), F('X1', Item.grid, alternate_name='G0', observed=True), F('X2'), F('X3'), F('CID', Item.coord),
                                                F('S'), F('OCID', Item.coord), F('S1'), F('S2'), F('S3')], card_tag=Tag.eSpring),
    'CONM2': class_factory('CONM2', Item.elem, [F(), F(), F('grids', Item.grid, Seq.list, 1, update_grid=True, observed=True), F('CID', Item.coord), F('M'), F('X1'), F('X2'), F('X3'), F(),
                                                F('I11'), F('I21'), F('I22'), F('I31'), F('I32'), F('I33')], card_tag=Tag.eMass),
    'PLOTEL': class_factory('PLOTEL', Item.elem, [F(), F(), F('grids', Item.grid, Seq.list, 2, update_grid=True, observed=True)], card_tag=Tag.ePlot),
    # Properties
